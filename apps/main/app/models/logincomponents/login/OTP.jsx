...
export default function OTP({ RouteRenderPages, EmailOrPhone }) {
      ...
      if (EmailOrPhone === "Phone") {
        ...
        mutateVerifyOTP.mutate(data, {
          onSuccess: (response) => {
            ...
            RouteRenderPages("ForgotPassword");
          },
          onError: (error) => {
            console.error(error);
  ...
  const resendOTP = (e) => {
    ...
    if (resendCounter === 0) {
      ...
      const phonedata = localStorage.getItem("phonenumberlogin");
      const emaildata = localStorage.getItem("emailidlogin");
      const contrycodelogin = localStorage.getItem("phonecountrycodelogin");
      ...
      if (EmailOrPhone === "Email") {
        ...
        mutateSendOTP.mutate(data, {
          ...
          onError: (error) => {
            console.error(error);
            alert("Could not send OTP via Email");
          },
        });
      } else {
        let data = {
          countryCode: contrycodelogin, // if channel == "sms", please provide
