...
import BottomFrontSVG from "./BottomFrontSVG";
import BottomBackSVG from "./BottomBackSVG";
...
function AddBottom({
  ...
  const [bottomFront, setBottomFront] = useState(null);
  const [bottomBack, setBottomBack] = useState(null);
  ...
  useEffect(() => {
    if (mode === "edit" && bottomData) {
      setBottomName(bottomData.name);
      const topBackPhoto = bottomData.photos.find(
        (photo: { file_bottom_back?: string }) => photo.file_bottom_back,
      );
      if (topBackPhoto) {
        setBottomBack(topBackPhoto.file_bottom_back);
      }

      // Find the photo URL for topFront
      const topFrontPhoto = bottomData.photos.find(
        (photo: { file_bottom_front?: string }) => photo.file_bottom_front,
      );
      if (topFrontPhoto) {
        setBottomFront(topFrontPhoto.file_bottom_front);
      }
    }
  }, [mode, bottomData]);
  ...
  const handleNextClick = () => {
    if (bottomFront && bottomBack) {
      ...
      data.append("apparel_position", "BOTTOM");
      if (mode === "edit") {
        data.append("apparel_id", bottomData.id);
      }
      ...
      appendFileOrPhoto("file_bottom_front", bottomFront, "file_bottom_front");
      appendFileOrPhoto("file_bottom_back", bottomBack, "file_bottom_back");
      ...
      mutatetopUpload.mutate(data, {
        onSuccess: (response) => {
          ...
          onCancel();
          if(setApparelsArray){
            setApparelsArray((currentBottomArray: any) => [...currentBottomArray, response]);
  ...
  const mutatetopUpload = useMutation({
    mutationKey: ["upload-bottom"],
    mutationFn: UploadApparel,
  });

  const mutatedeleteApparel = useMutation({
    mutationKey: ["delete-bottom"],
    mutationFn: deleteApparel,
  });

  const deleteApparelEdit = () => {
    const bottomDataDelete = new FormData();
    bottomDataDelete.append("fashion_kit_id", bottomData.fashion_kit_id);
    bottomDataDelete.append("workspace_id", bottomData.workspace_id);
    bottomDataDelete.append("apparel_id", bottomData.id);

    mutatedeleteApparel.mutate(bottomDataDelete, {
      onSuccess: (response) => {
        console.log("response :", response);
        onCancel();
        refetch();
        if(setApparelsArray){
          setApparelsArray((prevArray:any) => prevArray.filter((item:any) => item.id !== bottomData.id));
  ...
  return (
      ...
      <div className="flex flex-col">
        ...
        <div
          className={`flex justify-center items-center gap-7 mt-6 ${bottomFront !== null && bottomBack !== null && "mb-5"}`}
        >
          <UploadButton
            id="bottomfront"
            setUploadedFile={setBottomFront}
            disabled={disabled}
            uploadedFile={bottomFront}
            BackgroundSvg={BottomFrontSVG}
            title={"Bottom Front"}
            svgStyle={{
              zIndex: 0,
              position: "absolute",
          ...
          <UploadButton
            id="bottomback"
            setUploadedFile={setBottomBack}
            disabled={disabled}
            uploadedFile={bottomBack}
            BackgroundSvg={BottomBackSVG}
            title={"Bottom Back"}
            svgStyle={{
              zIndex: 0,
              position: "absolute",
        ...
        {(bottomFront === null || bottomBack === null) && (
          ...
          {mode === "edit" ? (
            <Button
              className={`font-semibold h-[37px] w-32  disabled:bg-nyx-gray-1 disabled:text-black disabled:cursor-not-allowed ${
                bottomFront && bottomBack && BottomName != ""
