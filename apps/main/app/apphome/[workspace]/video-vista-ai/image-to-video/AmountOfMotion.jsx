...
const AmountOfMotion = ({ tab, setBrandTab, updatedata,imageUploaded ,tabControl,reset}) => {
  const [RangeSliderValue, setRangeSliderValue] = useState(20);


  useEffect(()=>{
    setRangeSliderValue(20)
  },[reset])
  const AmountofMotionValue = () => {
    updatedata("AmountOfMotion", RangeSliderValue);
    if(tabControl==="1"){
      setBrandTab("")
    }
    else{
  ...
  const OpenAnimation =()=>{
    if(imageUploaded===0){
      ...
      if(tab===IMAGE_TO_VIDEO_TABS.MOTION)
      {
        setBrandTab("")
      }
      else{
    ...
    <>
      <div className="bg-[#3B226F] rounded-lg">
        <h2 className="mb-0">
          <div
            className={`${
              tab === IMAGE_TO_VIDEO_TABS.MOTION
            ...
            // onClick={() => setBrandTab(IMAGE_TO_VIDEO_TABS.MOTION)}
        ...
        <motion.div
          ...
          animate={{
            height: tab === IMAGE_TO_VIDEO_TABS.MOTION ? "auto" : 0,
            opacity: 1,
          }}
          exit={{
            height: 0,
          ...
          <div className="py-3 md:px-7 sm:px-4 amountOfMotionn" >
            ...
            <div className="flex  justify-end"  >
              ...
              <p className={`text-16 font-medium ${RangeSliderValue ?'text-[#FFCB54]' : "text-[#8297BD]"}`}>
               {RangeSliderValue ? (<>{RangeSliderValue}%</>):(<>0%</>) }
              </p>
            </div>
            <div className="text-center mt-4 mb-2">
              {/* <Button
                className="rounded-full bg-[#F1BB2E] text-[#000000] px-[40px]"
                onClick={AmountofMotionValue}
              >
                Save
              </Button> */}
               { RangeSliderValue ? (
