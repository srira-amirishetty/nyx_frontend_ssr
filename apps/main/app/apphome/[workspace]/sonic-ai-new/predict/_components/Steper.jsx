...
const Steper = ({ selectedGenre, originalSuccess, showPredictLoading }) => {
  const [progressstep, setprogressstep] = useState(0);

  useEffect(() => {
    if(showPredictLoading){
      setprogressstep(3);
  ...
  }, [selectedGenre, originalSuccess,showPredictLoading]);

  return (
    <>
      <div className="flex flex-col overflow-y-auto">
        <p className="w-full text-white text-xl font-semibold my-5">
          Steps for Success Prediction
        </p>
        <ol className="overflow-hidden space-y-4 italic">
          ...
          <Step
            ...
            title={"Add/upload your music"}
            lineActive={progressstep >= 2}
          />
          <Step
            isActive={progressstep >= 3}
            title={"Get your virality prediction"}
