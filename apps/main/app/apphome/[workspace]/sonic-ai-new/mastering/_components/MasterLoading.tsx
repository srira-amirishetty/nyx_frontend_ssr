...
type Props = {
  ...
  file: File | null;
  refId: string;
  originalsongname: string | undefined;
};
...
const MasterModal = (
  { onProgressComplete = () => {}, file, refId, originalsongname }: Props = {
    file: null,
    refId: "",
    originalsongname: undefined,
  },
) => {
  ...
  const queryAnanlysis = useQuery({
    queryKey: ["analysis-mastring", refId],
    queryFn: async () => {
      if (refId) {
        const response = await masteringService({
          uploadId: refId,
  ...
  useEffect(() => {
    timerID.current = setTimeout(() => {
      if (Object.keys(queryAnanlysis?.data?.nyx_mastered || {}).length) {
        onEndHandler();
      } else {
        if (progress < 99) {
          const max = Math.min(progress + increase, 99);
          setProgress(max);
          circleUpdate(max);
          setTime((old) => old + 500);
        } else {
    ...
    <>
      <div className="w-full h-[75%] rounded-lg bg-[#28134B]">
        <div className="w-full p-3">
          <h2 className="text-white text-xl font-bold mb-2 w-full">
            Mastering in progress...
          </h2>
          <p className="text-[#FFF] text-base font-[400]">
            Audio File Uploaded{" "}
            <span className="text-nyx-blue font-bold">{originalsongname}</span>
          </p>
        </div>
