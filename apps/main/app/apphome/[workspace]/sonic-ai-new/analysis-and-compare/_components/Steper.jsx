...
const Steper = ({ selectedGenre, originalSuccess, referenceSuccess,showAnalysisLoading }) => {
  ...
  }, [selectedGenre, originalSuccess, referenceSuccess,showAnalysisLoading]);

  return (
    <>
      <div className="flex flex-col overflow-y-auto">
        <p className="w-full text-white text-xl font-semibold my-5">
        Steps for Analyze and Compare
        </p>
        <ol className="overflow-hidden space-y-4 italic">
          <Step
            isActive={progressstep >= 1}
            title={"Select your music genre"}
            lineActive={progressstep >= 1}
          />
          <Step
            isActive={progressstep >= 2}
            title={"Select or add original music"}
            lineActive={progressstep >= 2}
          />
          <Step
            isActive={progressstep >= 3}
            title={"Select/add one or more reference music"}
            lineActive={progressstep >= 3}
          />
          <Step
            isActive={progressstep >= 4}
            title={"Get your analysis report in an interactive graph"}
