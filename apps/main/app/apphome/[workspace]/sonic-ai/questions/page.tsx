...
import { getUploadedQuestionsService } from "@nyx-frontend/main/services/uploadService";
...
function Question() {
  ...
  const { data, isPending } = useQuery({
    ...
    queryFn: async () =>
      search.get("processId")
        ? getUploadedQuestionsService(search.get("processId"))
        : null,
    retry: false,
  });
  ...
  const onLastClose = () => {
    setModal(false);
    // router.push("/upload/plans?ref=" + search.get("processId"))
    {
      hasActivePlan
        ? router.push(`/apphome/${workspace}/sonic-ai/reports?ref=` + search.get("processId"))
  ...
  return (
    <section className="my-10">
      <div className="mx-auto w-[95%]  h-full md:h-[80%] rounded-3xl bg-[#14264e] py-2 px-4">
        <div className="flex w-full justify-end ">
          <Link
            href={`/apphome/${workspace}/sonic-ai/probability?ref=${search.get("processId")}`}
