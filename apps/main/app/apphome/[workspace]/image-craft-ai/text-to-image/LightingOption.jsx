...
import { CAMERA_TABS, LIGHTINGOPTIONSDATA } from "@nyx-frontend/main/utils/productConstants";
    ...
    <>
      <div className="bg-[#3B226F] rounded-lg">
        <h2 className="mb-0">
          <div
            className={`${
              cameraTab === CAMERA_TABS.LIGHTING_OPTIONS
            ...
            onClick={() => setCameraTab(CAMERA_TABS.LIGHTING_OPTIONS)}
            aria-expanded="true"
        ...
        <motion.div
          ...
          animate={{
            height: cameraTab === CAMERA_TABS.LIGHTING_OPTIONS ? "auto" : 0,
            opacity: 1,
          }}
          exit={{
            height: 0,
              ...
              <div className="w-full flex flex-col gap-4">
                <div className="w-full flex gap-3">
                  <div className="w-full grid grid-cols-4 grid-flow-row gap-2">
                    {LIGHTINGOPTIONSDATA.map((item, index) => (
                      <button
                        key={item.id}
                        className={`flex flex-col transition-colors text-sm cursor-pointer text-center rounded-md hover:shadow-gray-800 shadow-none hover:shadow-md w-[93px] ${
                          clickLight && clickIndexLight === index
                            ? "border-2 border-[#5e32ff] bg-[#5e32ff]"
                            : "border-2 border-transparent"
                        }`}
                        onClick={() => lightOptions(item?.title, index)}
                        title={item.title}
                      >
                        <img
                          src={item.imgsrc}
                        ...
                        />
                        <div
                          className={`${clickLight && clickIndexLight === index ? "bg-[#5e32ff]" : "bg-black "} h-full text-white w-full rounded-b-md text-[10px]`}
