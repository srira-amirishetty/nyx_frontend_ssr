...
import "../text-to-image/index.css";
...
const brandPaintingOptions = [
  { value: "Edit with Brand Canvas", label: "Edit with Brand Canvas" },
...
const Imageslider = ({
  ...
  const handleDownload = async (image) => {
    ...
    // const imageUrl = imageurl[imageIndex]?.["imageLists"];
  ...
  const onChange = (option) => {
    if (option.value === "Edit with Brand Canvas") {
  ...
  return (
    ...
    >
      ...
      <div className=" rounded-lg w-full relative min-h-[200px] max-h-[350px] z-0 flex items-center">
          ...
          {imageurl.map((img) => (
            <div key={img.id} className="slick-slide">
              <div>
                ...
                {!img.isLoading && img.isError && <ImageError />}
                {!img.isLoading && !img.isError && (
                  <img
                    loading="lazy"
                    decoding="async"
                    className="max-h-[350px] max-w-full size-auto m-auto rounded-lg cursor-pointer"
                    alt="image"
                    src={img.image}
