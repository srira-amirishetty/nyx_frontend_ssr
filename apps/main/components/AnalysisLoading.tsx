...
const AnalysisLoading = (
  ...
  const queryAnanlysis = useQuery({
    ...
    queryFn: async () => {
      if (refId) {
        const response = await getAnalysisService(refId,Number(localStorage.getItem("workspace_id")));
  ...
  useEffect(() => {
    timerID.current = setTimeout(() => {
      ...
      }else if(queryAnanlysis?.data?.parameters?.original?.key && queryAnanlysis?.data?.struct_url){
        onEndHandler();
      } else {
        if (progress < 99) {
      ...
      <div className="fixed z-50 inset-0 m-auto flex justify-center items-end md:items-center">
        <div className="w-full md:w-3/4 rounded-lg bg-[#3B226F] py-10 px-4 md:px-10">
          ...
          <div className="w-full">
            ...
            <p className="text-[#FFF] text-sm md:text-base font-[400]">
              Audio File Uploaded{" "}
              <span className="text-nyx-blue font-bold">{fileName}</span>
            </p>
          </div>
