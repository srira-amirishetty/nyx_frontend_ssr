...
// export const generateThumbnail = (videoUrl: string, timeInSeconds: number) => {
...
const UploadAdCreative = ({
  ...
  const driveFileSelected = async (data: any, index: any) => {
    if (uploadType == "image") {
      ...
      try {
        const thumbnailFile = await generateThumbnail(videoUrl, 1); // Get thumbnail

        const formData = new FormData();
        formData.append("type", "thumbnail");
        formData.append(
          "workspace_id",
          String(localStorage.getItem("workspace_id")),
        );
          ...
          onSuccess: (response: any) => {
            setDriveVideoClickedArray((clickedData: any) => {
              const newItem = {
                ...
                thumbnail: response?.data?.signed_image_url,
              };

              if (
                clickedData.some((item: any) => isEqualVideo(item, newItem))
              ) {
                return clickedData.filter(
