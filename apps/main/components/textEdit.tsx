...
import { useEditorStyle } from "./_stores/store";
import { UseContextData } from "@nyx-frontend/main/hooks/usecontext";
import Select from "react-select";
import { colourStylesText } from "@nyx-frontend/main/utils/productStyle";
import { toast } from "react-toastify";
import { motion } from "framer-motion";
import BoldIcon from "@nyx-frontend/main/components/_components/BoldIcon";
import ItalicIcon from "@nyx-frontend/main/components/_components/ItalicIcon";
import UnderlineIcon from "@nyx-frontend/main/components/_components/UnderlineIcon";
import { SketchPicker } from "react-color";
import BgColorIcon from "@nyx-frontend/main/components/_components/BgColorIcon";
import SpellCheckIcon from "@nyx-frontend/main/components/_components/SpellCheckIcon";
import TextColorIcon from "@nyx-frontend/main/components/_components/TextColorIcon";

const TextEditing = () => {
  const fonts = useFont();
  const [count, setCount] = useState(25);
  const { activeCanvas } = useEditorStyle(
    useShallow((state) => ({ activeCanvas: state.activeCanvas }))
  );
  const [currentTextClick, setTextClick] = useState(0);
  const {
    setTrigger,
    toggleIcon,
    setToggleIcon,
  ...
  const onBgChangeComplete = (color: { hex: string }) => {
    setColor(color.hex);
    toggleIcon.bg_color = color.hex;
    setCount(currentTextClick + 1);
    setToggleIcon({ ...toggleIcon, toggleIcon });
    setTrigger(`bgColorPick${currentTextClick}`);
  };

  const onChangeTextComplete = (color: { hex: string }) => {
    setTextColor(color.hex);
    toggleIcon.text_color = color.hex;
    setToggleIcon({ ...toggleIcon, toggleIcon });
    setCount(currentTextClick + 1);
    setTrigger(`textColorPick${currentTextClick}`);
  };

  const onItalicClick = () => {
    setTrigger(`italic${currentTextClick}`);
    setTextClick(currentTextClick + 1);
  };

  const onBoldClick = () => {
    setTrigger(`bold${currentTextClick}`);
    setTextClick(currentTextClick + 1);
  };

  const onUnderlineClick = () => {
    setTrigger(`underline${currentTextClick}`);
    setTextClick(currentTextClick + 1);
  };
  ...
  return (
    <>
      <button
        onClick={() => {
          if (imageUploaded) {
            setTrigger(`addText${currentTextClick}`);
            setTextClick(currentTextClick + 1);
          } else {
            toast.error("Upload Image and retry!");
          }
        }}
        className="text-nyx-yellow hover:bg-nyx-sky hover:text-black bg-black w-full mt-10 text-bold p-2 text-center rounded-lg"
      >
        Add a text box
      </button>
        ...
        <>
          <div className="flex flex-row gap-2 mt-10">
            <div className="w-2/3">
              <Select
                onChange={(event: any) => {
                  toggleIcon.font = event.value;
                  setToggleIcon({ ...toggleIcon, toggleIcon });
                  setTrigger(`fontFamily${currentTextClick}`);
                  setTextClick(currentTextClick + 1);
                }}
                className="text-sm md:text-base"
                options={fonts.map((font) => ({ label: font, value: font }))}
                styles={colourStylesText}
                components={{
            ...
            <div className="flex w-1/3">
              <div
                ...
                onClick={() => {
                  if (fontSized > 1) {
                    setTrigger(`fontSize${currentTextClick}`);
                    setToggleIcon({ ...toggleIcon, toggleIcon });
                    setTextClick(currentTextClick + 1);
                    setFontSize(fontSized - 1);
                  }
                }}
              >
                -
          ...
          <div className="flex flex-col gap-2 mt-10">
            <div className="grid grid-cols-5 gap-4">
              <div className="">
                {" "}
                <p className="text-white pt-1 text-[16px]">Alignment</p>
              </div>
              <div className="cursor-pointer">
                ...
                <div
                  className=""
                  onClick={() => {
                    setTrigger(`leftalignment${currentTextClick}`);
                    setTextClick(currentTextClick + 1);
                  }}
                >
                  <svg
              ...
              <div
                onClick={() => {
                  setTrigger(`centeralignment${currentTextClick}`);
                  setTextClick(currentTextClick + 1);
                }}
              ...
              >
                <svg
                  ...
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <rect y="0.25" width="73" height="37" rx="4" fill="#1D1138" />
                  <rect
                    x="25.3359"
                    y="12.7969"
                    width="22.3281"
              ...
              <div
                className="cursor-pointer"
                onClick={() => {
                  setTrigger(`rightalignment${currentTextClick}`);
                  setTextClick(currentTextClick + 1);
                }}
              >
                <svg
              ...
              <div
                className="cursor-pointer"
                onClick={() => {
                  setTrigger(`justifyalignment${currentTextClick}`);
                  setTextClick(currentTextClick + 1);
                }}
              >
                <svg
            ...
            <div className="grid grid-cols-5 gap-4">
              <div className="w-[20%]">
                {" "}
                <p className="text-white text-[16px] pt-1">Decoration</p>
              </div>
              <motion.button whileTap={{ scale: 0.9 }} onClick={onBoldClick}>
                <BoldIcon
                  active={activeCanvas?.fontWeight === "bold"}
                  className="w-[73px]"
                />
              </motion.button>
              <motion.button whileTap={{ scale: 0.9 }} onClick={onItalicClick}>
                <ItalicIcon
                  active={activeCanvas?.fontStyle === "italic"}
                  className="w-[73px]"
                />
              </motion.button>
              <motion.button
              ...
              >
                <UnderlineIcon
                  active={activeCanvas?.["textDecoration"] === "underline"}
                  className="w-[73px]"
                />
              </motion.button>
              <div
                className="cursor-pointer"
                onClick={() => {
                  setTrigger(`sign${currentTextClick}`);
                  setTextClick(currentTextClick + 1);
                }}
              ></div>
            </div>
            ...
            <div className="grid grid-cols-5 gap-4">
              <div className="w-[20%]">
                {" "}
                <p className="text-white pt-1 text-[16px]">Case</p>
              </div>
              <div
                className="cursor-pointer"
                onClick={() => {
                  setTrigger(`uppercase${currentTextClick}`);
                  setTextClick(currentTextClick + 1);
                }}
              >
                <svg
                  ...
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <rect y="0.25" width="77" height="37" rx="4" fill="#1D1138" />
                  <path
              ...
              <div
                className="cursor-pointer"
                onClick={() => {
                  setTrigger(`lowercase${currentTextClick}`);
                  setTextClick(currentTextClick + 1);
                }}
              >
                <svg
              ...
              <div
                className="cursor-pointer"
                onClick={() => {
                  setTrigger(`Titlecase${currentTextClick}`);
                  setTextClick(currentTextClick + 1);
                }}
              >
                <svg
          ...
          <div className="mt-10">
            <div className="flex gap-4">
              <div className="bg-[#50387B] flex flex-col justify-center cursor-pointer w-full rounded-md overflow-hidden">
                <div
                  className="mx-auto h-12 mt-3"
                ...
                <div
                  className="text-white w-full text-center bg-nyx-blue-3 h-7"
                  onClick={() => setTextOpen(!Textopen)}
                >
                  Text color
                </div>
                ...
                <div className="absolute -mt-96">
                  {Textopen && (
                    <SketchPicker
                      key={"text-sketch-picker"}
                      color={Textcolor}
                      onChangeComplete={onChangeTextComplete}
                    />
                  )}
                </div>
              </div>
              ...
              <div className="bg-[#50387B] flex flex-col justify-center cursor-pointer w-full rounded-md overflow-hidden">
                <div className="mx-auto h-12 mt-3">
                  <SpellCheckIcon className="w-11" />
                </div>

                <div className="text-white w-full text-center bg-nyx-blue-3 h-7">
                  Spell Check
                </div>
              </div>
              <div></div>
