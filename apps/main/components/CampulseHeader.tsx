...
export default function Header({ header_bg = "" }: { header_bg?: string }) {
  ...
  const sendToDashBoard = () => {
    if (workspace) {
      navigate.push(`/apphome/${workspace}/admanager/dashboard?view=graph`);
    }
  };
  ...
  const logintocampulse = () => {
    localStorage.setItem("Login-Type", "campulse");
    navigate.push(process.env.NEXT_PUBLIC_APP_URL + `/apphome/login`);
  };

  return (
    <>
      <nav
        className={`text-white md:px-2 sm:px-4 h-[60px] md:h-[70px] w-full  z-40 top-0 left-0 fixed bg-tranparent `}
      >
        <div
          className="lg:flex hidden m-auto w-[90%] rounded-full px-6 mt-[40px]"
          style={navbarStyle}
        >
            ...
            {isLoggedIn ? (
              <div>
                <div className="flex gap-4 justify-end ">
                  <div className="w-[128px] " onClick={sendToDashBoard}>
              ...
              <div>
                <div className="flex gap-4">
                  ...
                  <div className="w-full" onClick={logintocampulse}>
                    <TAIL_BUTTON
                      style="w-full"
                      disable={false}
                    //onClick={handleLogin}
        ...
        <div
          className="relative md:hidden  z-40 flex flex-row px-8 mt-[10px] rounded-full  h-[60px]"
          style={navbarStyle}
        >
          {/* Buttons for mobile */}
          <div className="absolute top-1 md:top-3 lg:top-5 left-7">
            <Link href="/">
              <Image
                src={logo}
                width={70}
          ...
          <button
            className="flex flex-row gap-4 absolute top-4 right-5 md:top-6"
          ...
          {/* {isLoggedIn && process.env.NEXT_PUBLIC_ENVIRONMENT === "test" ? (
            <div
        ...
        >
          <div className="flex flex-col mb-[300px] bg-[#261746] w-full text-[14px] font-semibold">
            ...
            <Link href="/" className=" hover:bg-[#7048D7] w-full">
              <div className="mt-4 mb-2 ml-4">
                {/* <p className="text-[14px] font-bold" onClick={openPopup}>
                  Models
                </p> */}
                {<Waitlist onClose={closePopup} isPopupOpen={isPopupOpen} />}
              </div>
